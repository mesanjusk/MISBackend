...
const handleApiCallNode = async ({ flow, node, session }) => {
  const config = node.apiConfig || {};
  if (!config.url) {
    return { nextNodeId: config.onErrorNodeId || node.defaultNextNodeId || getFallbackNextNodeId(flow, node) };
  }
...
const processSession = async ({ flow, session, incomingText, incomingReplyId }) => {
  ...
  const replyInput = normalizeText(incomingReplyId);
  let safetyCounter = 0;
  ...
  while (currentNodeId && safetyCounter < 25) {
    ...
    if (node.type === 'button') {
      ...
      const selectedOption = options.find((item, index) => {
        ...
        return (
          ...
          normalizeKeyword(replyInput) === normalizeKeyword(item.id) ||
    ...
    if (node.type === 'api_call') {
      const result = await handleApiCallNode({ flow, node, session });
  ...
  return {
    handled: outgoingMessages.length > 0 || !waitingForUserInput || session.isCompleted,
    outgoingMessages,
    session,
  };
};
...
const processIncomingMessageFlow = async ({ payload, sendText }) => {
  ...
  const result = await processSession({
    ...
    incomingText: isNewSession ? '' : incomingText,
  });
